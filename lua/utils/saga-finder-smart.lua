-- Code copied and modified from https://github.com/glepnir/lspsaga.nvim/blob/main/lua/lspsaga/finder.luahttps://github.com/glepnir/lspsaga.nvim/blob/main/lua/lspsaga/finder.lua
...
function Finder:loading_bar()
	...
	spin_timer:start(
		...
		vim.schedule_wrap(function()
			...
			if
				(self.request_status[methods[1]] and self.request_status[methods[2]]) and not spin_timer:is_closing()
			then
				spin_timer:stop()
				spin_timer:close()
				if api.nvim_buf_is_loaded(spin_buf) then
				...
				window.nvim_close_valid_window(spin_win)
				-- if cursor not on definition and only one definition go to definition
				-- if cursor on definition and only one reference go to that reference
				if self:only_one_definition_present() then
					local only_definition = self.request_result[methods[1]][1]
...
function Finder:only_one_definition_present()
	local definition_result = self.request_result[methods[1]]
...
function Finder:current_cursor_not_on_result(result)
	...
	local target_range = result.targetSelectionRange or result.range
	local target_row = target_range.start.line + 1
	local target_col_start = target_range.start.character + 1
	local target_col_end = target_range["end"].character + 1

	local current_bufnr = vim.fn.bufnr("%")
	local current_range = api.nvim_win_get_cursor(0)
	...
	return current_row ~= target_row
		or target_bufnr ~= current_bufnr
		or current_col < target_col_start
		or current_col > target_col_end
end
...
-- TODO if no definition do not show definitions
...
function Finder:create_finder_contents(result, method)
	...
	for _, res in ipairs(result) do
		...
		local link = self:uri_to_link(uri)
		...
		local range = res.targetSelectionRange or res.range
...
function Finder:uri_to_link(uri)
...
function Finder:lsp_finder_highlight()
	...
	api.nvim_buf_add_highlight(self.bufnr, -1, "References", self.ref_scope[1], #icons.ref, #icons.ref + ref_len)
	api.nvim_buf_add_highlight(self.bufnr, -1, "ReferencesCount", self.ref_scope[1], #icons.ref + ref_len, -1)
end
