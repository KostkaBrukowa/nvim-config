...
function M.register_sources(configs, method)
	...
	for _, config in ipairs(configs) do
		...
		-- print(string.format("Received request to register [%s] as a %s source", name, type))
		if not source then
		-- print("Not a valid source: " .. name)
		elseif is_registered({ name = source.name or name, method = method }) then
		-- print(string.format("Skipping registering [%s] more than once", name))
		else
