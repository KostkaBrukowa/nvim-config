...
local function check_pr_exists()
  ...
  local handle =
    io.popen(string.format("gh pr view %s --json url,isDraft 2>/dev/null", current_branch))
  if not handle then
    return false, nil
  end
...
function M.post_push_pr_check()
  ...
  vim.defer_fn(function()
    ...
    if pr_exists and pr_data then
      ...
      vim.ui.select({ "Open PR in browser", "Cancel" }, {
      ...
      vim.ui.select({ "Open PR", "Open Draft PR", "Cancel" }, {
        prompt = string.format(
          "No PR found for '%s'. Create PR to '%s'?",
          current_branch,
          base_display
        ),
      }, function(choice)
